...
import {
  Eye, EyeOff, Check, X, Mail, Lock, Loader2, LogIn,
  Shield, AlertTriangle, Smartphone, Chrome, Globe
} from 'lucide-react';
...
const Login = () => {
  ...
  const isFormValid = uiState.emailValid && uiState.passwordValid && !uiState.isLocked && uiState.isOnline;
  ...
  useEffect(() => {
    const biometricAvailable = 'webauthn' in window || navigator.userAgent.includes('Mobile');
  ...
  useEffect(() => {
    const handleKeyPress = (e) => {
      if (uiState.focusedField === 'password') { // Only check for password field
  ...
  useEffect(() => {
    if (uiState.lockoutTime > 0) {
      ...
      setUiState(u => ({ ...u, isLocked: false, loginAttempts: 0, errorMessage: '' }));
    }
  ...
  useEffect(() => {
    ...
    setUiState(u => ({
      ...
      emailValid,
      passwordValid,
      passwordStrength: formData.password ? calculatePasswordStrength(formData.password) : 0
    }));
  ...
  if (!isFormValid || uiState.isLocked || uiState.isSubmitting) {
    setUiState(u => ({ ...u, errorMessage: 'Please ensure all fields are valid and account is not locked.' }));
    return;
  }
  ...
  try {
    ...
    if (data.role === USER_ROLES.STUDENT) {
      redirectPath = data.profileComplete ? '/student/dashboard' : '/student/profile-setup';
    } else if (data.role === USER_ROLES.TEACHER) {
      redirectPath = data.profileComplete ? '/teacher/dashboard' : '/teacher/profile-setup';
    } else {
      redirectPath = '/';
    }
  ...
  const handleSocialLogin = useCallback(async (type) => {
    setUiState(u => ({ ...u, isSubmitting: true, errorMessage: '' }));
    await new Promise(r => setTimeout(r, 1500));

    const mockUserData = {
      email: `social_${type}@example.com`,
      password: 'password123', // In a real app, this wouldn't be stored/needed for social login
      role: USER_ROLES.STUDENT, // Default new social logins to student role
      profileComplete: false, // Assume profile is not complete for new social users
      firstName: type === 'google' ? 'Social' : 'Biometric',
      lastName: 'User',
      // In a real app, you'd get a proper unique ID from the social provider
      id: 'social-user-' + Math.random().toString(36).substr(2, 9),
      studentProfileData: null,
      teacherProfileData: null,
    };

    const currentRegisteredUsers = getFromLocalStorage('registeredUsers', []);
    const existingSocialUser = currentRegisteredUsers.find(user => user.email === mockUserData.email);

    // If social user doesn't exist, "register" them
    if (!existingSocialUser) {
      currentRegisteredUsers.push(mockUserData);
      setToLocalStorage('registeredUsers', currentRegisteredUsers);
    }

    setToLocalStorage('currentUser', mockUserData);
    setToLocalStorage('lastLoginTime', new Date());

    // Determine redirect path based on the default role for social login (student)
    const redirectPath = mockUserData.profileComplete ? `/${mockUserData.role}/dashboard` : `/${mockUserData.role}/profile-setup`;

    console.log(`${type} social login successful, redirecting to: ${redirectPath}`);
    navigate(redirectPath);

    setUiState((u) => ({ ...u, isSubmitting: false }));
  }, [navigate]);
  ...
  const renderInput = useCallback((label, name, type = 'text', icon = null, toggleable = false) => {
    const val = formData[name];
    const isFocused = uiState.focusedField === name;
    const isValid = name === 'email' ? uiState.emailValid : (name === 'password' ? uiState.passwordValid : true);
    ...
    return (
      <div className="space-y-2 transform hover:scale-[1.02] transition-all duration-300">
        <label className={`block text-sm font-semibold text-slate-700 flex items-center gap-2 transition-colors duration-200 hover:text-violet-600`}>
          {icon && <span className="text-violet-500 transition-transform duration-200 hover:scale-110">{icon}</span>}
          {label} <span className="text-red-500">*</span>
        </label>
        <div className="relative">
          <input
            type={toggleable ? (uiState.showPassword ? 'text' : 'password') : type}
            name={name}
            value={val}
            ...
            className={`${inputBaseClasses} ${themeSpecificClasses} ${
              hasError ? 'border-red-400 bg-red-50 hover:shadow-red-100' :
              (isValid && val) ? 'border-emerald-400 bg-emerald-50 hover:shadow-emerald-100' :
              isFocused ? 'border-violet-400 bg-violet-50 shadow-violet-100' : ''
            }`}
          ...
          {val && !toggleable && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2 transform hover:scale-110 transition-transform duration-200">
              {isValid ? <Check className="w-5 h-5 text-emerald-500" /> : <X className="w-5 h-5 text-red-500" />}
            </div>
          )}
        </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-gray-200 rounded-full h-2">
                <div
                  className={`h-2 rounded-full transition-all duration-300 ${getPasswordStrengthColor().split(' ')[0]}`}
                  style={{ width: `${(uiState.passwordStrength / 5) * 100}%` }}
                ></div>
              </div>
              <span className={`text-xs font-medium ${getPasswordStrengthColor().split(' ')[1]}`}>
        ...
        {name === 'password' && uiState.showCapsLockWarning && isFocused && (
          <div className="flex items-center gap-2 text-amber-600 text-sm bg-amber-50 px-3 py-2 rounded-lg" aria-live="polite">
            <AlertTriangle className="w-4 h-4" />
            <span>Caps Lock is on</span>
          </div>
        )}

        {errorMessage && <p id={`${name}-error`} className="text-sm text-red-600 flex items-center gap-1" aria-live="assertive">
          <X className="w-4 h-4" />
          {errorMessage}
        </p>}
      </div>
    );
  }, [formData, uiState, handleInputChange, getPasswordStrengthColor, getPasswordStrengthText, inputBaseClasses, themeSpecificClasses]);
    ...
    <div className={`min-h-screen flex items-center justify-center bg-gradient-to-br from-violet-50 via-purple-50 to-indigo-50 px-4 py-8`}>
      <div className={`max-w-md w-full bg-white rounded-3xl shadow-xl overflow-hidden transform hover:scale-[1.02] transition-all duration-500 hover:shadow-2xl`}>
        <div className="bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 px-8 py-8 text-white relative overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-r from-violet-600/90 via-purple-600/90 to-indigo-600/90 group-hover:from-violet-700/90 group-hover:via-purple-700/90 to-indigo-700/90 transition-all duration-500"></div>
          ...
          <div className="absolute top-4 right-4 flex gap-2 z-20">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${uiState.isOnline ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
              <Globe className={`w-4 h-4 ${uiState.isOnline ? 'text-green-300' : 'text-red-300'}`} />
            </div>
          </div>
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="text-2xl font-bold transform group-hover:scale-105 transition-transform duration-300">Welcome Back</h1>
                <p className="text-violet-200 text-sm">Sign in to continue learning</p>
              </div>
            </div>

            {uiState.lastLoginTime && (
              <div className="text-violet-200 text-xs opacity-75">
                Last login: {uiState.lastLoginTime.toLocaleDateString()} at {uiState.lastLoginTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          {!uiState.isOnline && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-red-800 font-medium text-sm">No internet connection</p>
                <p className="text-red-600 text-xs">Please check your connection and try again</p>
              </div>
            </div>
          )}

          {uiState.isLocked && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3" aria-live="assertive">
              <Shield className="w-5 h-5 text-red-500" />
              <div>
                <p className="text-red-800 font-medium text-sm">Account temporarily locked</p>
                <p className="text-red-600 text-xs">Try again in {uiState.lockoutTime} seconds</p>
              </div>
            </div>
          )}
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3" aria-live="assertive">
              ...
              <div>
                ...
                {uiState.loginAttempts > 0 && !uiState.isLocked && (
                  <p className="text-red-600 text-xs">Attempts remaining: {3 - uiState.loginAttempts}</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            <button
              ...
              disabled={uiState.isSubmitting || uiState.isLocked || !uiState.isOnline}
              className="w-full py-3 px-4 border border-slate-300 rounded-xl font-medium flex justify-center items-center space-x-3 transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:border-slate-400 bg-white hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          {renderInput('Email Address', 'email', 'email', <Mail className="w-4 h-4" />)}
          {renderInput('Password', 'password', 'password', <Lock className="w-4 h-4" />, true)}
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center space-x-2 cursor-pointer group">
              ...
              />
              <span className={`text-sm text-slate-700 group-hover:text-violet-600 transition-colors duration-200`}>Remember me</span>
            </label>
